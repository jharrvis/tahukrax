<div class="space-y-6" x-data="{ 
    ...
    <section class="space-y-3">
        ...
        <div x-data="{ open: false }">
            ...
            <div x-show="open" x-collapse class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                @foreach($allPackages as $pkg)
                    ...
                    @endphp
                    <div class="bg-white rounded-xl border border-gray-200 hover:border-orange-500 hover:shadow-md transition-all flex items-center gap-3 p-3 cursor-pointer group"
                         wire:click="addPackage({{ $pkg->id }})"
                         @click="open = false">
                        <!-- Image Left -->
                        <div class="w-16 h-16 bg-gray-50 rounded-lg overflow-hidden flex-shrink-0 flex items-center justify-center border border-gray-100 group-hover:border-orange-200">
                            @if($imgUrl)
                        ...
                        <!-- Name & Price Center -->
                        <div class="flex-1 min-w-0">
                            <h4 class="text-gray-800 font-bold text-sm truncate group-hover:text-orange-600 transition-colors">{{ $pkg->name }}</h4>
                            <p class="text-orange-500 text-xs font-bold">Rp {{ number_format($pkg->price / 1000000, 1) }} Jt</p>
                        </div>
                        <!-- Add Button Right -->
                        <div class="w-8 h-8 rounded-full bg-gray-50 text-gray-400 group-hover:bg-orange-500 group-hover:text-white flex items-center justify-center flex-shrink-0 transition-colors shadow-sm">
                            <i class="fas fa-plus text-xs"></i>
                        </div>
                    </div>
                @endforeach
