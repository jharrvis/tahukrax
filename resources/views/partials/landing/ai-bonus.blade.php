<section class="py-16 md:py-24 bg-gray-900 border-t border-gray-800 relative overflow-hidden">
    <!-- Background Accents -->
    <div
        class="absolute top-0 right-0 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none">
    </div>
    <div
        class="absolute bottom-0 left-0 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 pointer-events-none">
    </div>

    <div class="max-w-7xl mx-auto px-4 relative z-10">
        <!-- Header Section -->
        <div class="text-center mb-16">
            <div
                class="inline-flex items-center justify-center p-3 bg-gray-800 rounded-2xl mb-6 shadow-lg border border-gray-700 animate-bounce-slow">
                <i
                    class="fas fa-robot text-4xl text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500"></i>
            </div>
            <h2 class="text-3xl md:text-5xl font-bold text-white mb-6">
                Bonus <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">Asisten
                    AI</span> RC Go
            </h2>
            <div
                class="bg-gray-800/50 backdrop-blur-sm border border-gray-700 p-6 md:p-8 rounded-3xl max-w-4xl mx-auto shadow-2xl">
                <p class="text-gray-300 text-lg leading-relaxed">
                    Untuk membantu Anda menjalankan usaha dengan lebih rapi dan konsisten, <span
                        class="text-white font-bold">RC Go menghadirkan Asisten AI senilai <span
                            class="text-green-400 border-b-2 border-green-500/50">Rp18.900.000</span> yang bisa
                        digunakan <span
                            class="text-yellow-400 font-extrabold bg-yellow-400/10 px-2 py-0.5 rounded">GRATIS</span>.</span>
                    <br><br>
                    Asisten AI ini siap menemani aktivitas harian usaha Anda, mulai dari membantu operasional hingga
                    promosi, sehingga usaha terasa lebih ringan dan mudah dijalankan, bahkan jika Anda baru pertama kali
                    memulai.
                </p>
            </div>
        </div>

        <!-- AI Roles Grid -->
        <div class="mb-16">
            <h3 class="text-2xl font-bold text-white mb-10 text-center flex items-center justify-center gap-3">
                <i class="fas fa-layer-group text-blue-500"></i> Beberapa peran <span
                    class="italic underline decoration-blue-500 underline-offset-4">Asisten AI RC Go</span> :
            </h3>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <!-- Racer -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-blue-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-blue-500/10 p-3 rounded-xl group-hover:bg-blue-500/20 transition-colors">
                            <i class="fas fa-flag-checkered text-blue-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Racer</h4>
                            <p class="text-xs text-blue-300 italic mb-2">Gesit & Tangkas</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI spesialis manajemen durasi sewa, sistem antrean penyewa, dan pengingat waktu main.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Pitstop -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-red-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-red-500/10 p-3 rounded-xl group-hover:bg-red-500/20 transition-colors">
                            <i class="fas fa-tools text-red-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Pitstop</h4>
                            <p class="text-xs text-red-300 italic mb-2">Siaga & Teliti</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI pengecekan kondisi unit (baterai, fisik mobil) sebelum dan sesudah disewakan agar
                                aset tetap aman.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Vibe -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-purple-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-purple-500/10 p-3 rounded-xl group-hover:bg-purple-500/20 transition-colors">
                            <i class="fas fa-camera text-purple-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Vibe</h4>
                            <p class="text-xs text-purple-300 italic mb-2">Atraktif & Seru</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI pembuat konten promosi di lokasi (dokumentasi keseruan penyewa) untuk menarik orang
                                datang ke booth.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Scripta -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-teal-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-teal-500/10 p-3 rounded-xl group-hover:bg-teal-500/20 transition-colors">
                            <i class="fas fa-file-alt text-teal-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Scripta</h4>
                            <p class="text-xs text-teal-300 italic mb-2">Ramah & Informatif</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI pengolah kata untuk brosur harga, aturan sewa, dan broadcast promo ke database
                                pelanggan.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Boost -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-orange-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-orange-500/10 p-3 rounded-xl group-hover:bg-orange-500/20 transition-colors">
                            <i class="fas fa-rocket text-orange-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Boost</h4>
                            <p class="text-xs text-orange-300 italic mb-2">Persuasif & Viral</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI perancang promo paket (misal: main 3x gratis 1x) dan strategi agar arena selalu ramai
                                saat akhir pekan.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Buddy -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-pink-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-pink-500/10 p-3 rounded-xl group-hover:bg-pink-500/20 transition-colors">
                            <i class="fas fa-hands-helping text-pink-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Buddy</h4>
                            <p class="text-xs text-pink-300 italic mb-2">Sabar & Edukatif</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI konsultan cara pakai untuk pemula/anak-anak agar mobil tidak mudah rusak dan penyewa
                                senang.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Tracka -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-cyan-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-cyan-500/10 p-3 rounded-xl group-hover:bg-cyan-500/20 transition-colors">
                            <i class="fas fa-calendar-check text-cyan-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Tracka</h4>
                            <p class="text-xs text-cyan-300 italic mb-2">Efisien & Teratur</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI pengelola jadwal lokasi (jika berpindah-pindah event/CFD) dan laporan pendapatan
                                harian.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Scope -->
                <div
                    class="bg-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-indigo-500/50 transition-all hover:bg-gray-800/80 group">
                    <div class="flex items-start gap-4">
                        <div class="bg-indigo-500/10 p-3 rounded-xl group-hover:bg-indigo-500/20 transition-colors">
                            <i class="fas fa-bullseye text-indigo-400 text-xl"></i>
                        </div>
                        <div>
                            <h4 class="text-white font-bold text-lg mb-1">Scope</h4>
                            <p class="text-xs text-indigo-300 italic mb-2">Target Luas</p>
                            <p class="text-gray-400 text-sm leading-relaxed">
                                AI spesialis iklan lokal (Meta Ads radius 5-10km) untuk menargetkan orang tua atau anak
                                muda di sekitar lokasi usaha.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Conclusion Box -->
        <div
            class="bg-gradient-to-r from-blue-900/40 to-purple-900/40 border border-blue-500/30 rounded-3xl p-8 text-center max-w-5xl mx-auto backdrop-blur-md">
            <p class="text-lg md:text-xl text-gray-200">
                👉 Dengan Asisten AI, usaha RC Go Anda menjadi <span class="text-white font-bold">lebih tertata, lebih
                    aktif promosi, dan lebih mudah dijalankan, bahkan untuk pemula.</span>
            </p>
        </div>
    </div>
</section>